...
/*
 *This function modelates the information of the menu planning problem
 *particularly it add the feasibility per day and global with the variability of the food.
 *Thus, "objFeas" is a vector of size two where the first refers to daily feasibility and the second to global feasibility.
 *"objVar" refers to the variability with size 2*N_TIMES (2*6), the first N_TIMES (or six) elements refers to the distance to closest days in whcih a food is repeated
 *the last N_TIMES refers to the dcn in terms of category.
 *This function ranks the variabilitys as is setted in the "global.h" and "global.cpp" files, where is indicated the configuration of all the program
 * */
...
//	  normalized *=g_rank[i];
	  ...
	  double normalizedCat = (1.0-((dcnCat+repCat)/((double) nDias)));
	  normalizedCat = g_l_rank_category[i] + normalizedCat*(g_u_rank_category[i]-g_l_rank_category[i]-0.1);
//	  normalizedCat *=g_rank_category[i];
