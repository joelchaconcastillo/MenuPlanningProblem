...
//	     if(((int)elapsedTime) % 60 == 0) cout << elapsedTime<<endl;
	     ite++;
        calculateFeasibilityDegree(x_var, objFeasibility[0], objFeasibility[1]);
        calculateVariability(x_var, objVariability);
...
//	leftShift(x_var, objFeasibility, objVariability);
//        calculateVariability(x_var, objVariability);
//	Improvement(x_var, objFeasibility, objVariability);
...
//   auto vb=bestVar;
//
//   double fit=modelation(bestFeas, bestVar);
//   double fitb=fit;
//   auto targetDay=badDaysVar;
//   for(auto bd:targetDay)
//   {
//   //for(int i = 0; i+K < nDias; i++){
//   for(int i = max(0, bd-K); i < min(bd+K, nDias); i++){
//   	  vector<int> perm;
//           for(int ii = i; ii < min(i+K, nDias); ii++)perm.push_back(ii);
//	   do{
//	      for(int j=0; j < perm.size(); j++){
//	        for(int jj = 0; jj < N_OPT_DAY; jj++){
//		   x[(i+j)*N_OPT_DAY+jj]=best_sol[perm[j]*N_OPT_DAY+jj];
//	        }
//	      }
//	      calculateVariability(x, v);
//	      auto nf=modelation(f,v);
//	      if( nf<fitb){
//		     fitb=nf;
//		     fb=f;
//		     vb=v;
...
//   bestFeas=fb;
//   bestVar=vb;
//   //cout <<setprecision(10)<<" " <<fit <<"----"<< fitb<<endl;
//   }
//}
void MPP::Improvement(vector<int> &best_sol, vector<double> &bestFeas, vector<pair<double, double> > &bestVar){
   int K=3;
...
void MPP::leftShift(vector<int> &best_sol, vector<double> &bestFeas, vector<pair<double, double> > &bestVar){
  auto cX=best_sol;
  auto cV=bestVar;
  auto bX=best_sol;
  calculateVariability(best_sol, bestVar);
  double bestScore = modelation(bestFeas, bestVar);
  for(int i = 0; i < nDias; i++){
     for(int j = 0; j < nDias; j++){
	    int dA=(i+j)%nDias, dB=j;
	  for(int k = 0; k < N_OPT_DAY; k++){
	    cX[dA*N_OPT_DAY+k]=best_sol[dB*N_OPT_DAY+k];
	  }
     }
     calculateVariability(cX, cV);
     double cScore= modelation(bestFeas, cV);
     if(cScore < bestScore){
	    bestScore=cScore;
	    bX=cX;
     }
