...
void MA::replacement_cdrw(){
   pair<int, int> lowestDCN(INT_MAX, 0);
    for(int i =0 ; i < N; i++){
       for(int j = 0; j<N; j++){
	if(i==j)continue;
  	   lowestDCN = min(lowestDCN, make_pair(population[i]->ind.getDistance(population[j]->ind), i));
       }
    }
    int mindist=INT_MAX;
    for(int j = 0; j  < N; j++){
  	   mindist = min(mindist, offspring.back()->ind.getDistance(population[j]->ind));
    }
    int idx = lowestDCN.second;
    if( mindist > lowestDCN.first && offspring.back()->ind.fitness < population[idx]->ind.fitness){
		swap(offspring.back(), population[idx]);
		for(int i = 0; i < offspring.size(); i++) delete offspring[i];
		offspring.clear();
		return;
    }
    ///OTHERWISE APPLY RW PROCEDURE REOVE WORST FITNESS
   pair<double, int> worstfitness(-DBL_MAX, 0);
   for(int i = 0; i < N; i++){
	worstfitness = max(make_pair(population[i]->ind.fitness,i), worstfitness);
   }
   pair<double, int> bestfitnessOFF(DBL_MAX, 0);
   for(int i = 0; i < offspring.size(); i++){
	bestfitnessOFF = min(bestfitnessOFF, make_pair(offspring[i]->ind.fitness, i));
   }
   if(worstfitness.first > bestfitnessOFF.first)
	swap(offspring[bestfitnessOFF.second], population[worstfitness.second]);
   for(int i = 0; i < offspring.size(); i++) delete offspring[i];
     offspring.clear();
}
