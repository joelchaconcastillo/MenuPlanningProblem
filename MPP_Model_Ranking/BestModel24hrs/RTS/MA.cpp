...
void MA::replacement_rts(){
   int CF=5;   
   for(int i= 0; i < offspring.size(); i++){
      pair<int, int> nearest(INT_MAX,0);
      for(int j = 0; j < CF; j++){
	     int idx = rand()%N;
	     nearest= min(nearest, make_pair(offspring[i]->ind.getDistance(population[idx]->ind), idx));
      }
      if(offspring[i]->ind.fitness < population[nearest.second]->ind.fitness)
	      swap(offspring[i], population[nearest.second]);
     delete offspring[i];
   }
   offspring.clear();
}
