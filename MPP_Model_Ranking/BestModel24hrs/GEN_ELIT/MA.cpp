...
void MA::replacement_gen_elit(){
	priority_queue<pair<double, ExtendedIndividual*> >all;		
	//Join population and offspring multisets
	for (int i = 0; i < population.size(); i++){
		all.push(make_pair(-population[i]->ind.fitness, population[i]));
	}

	for (int i = 0; i < offspring.size(); i++){
		all.push(make_pair(-offspring[i]->ind.fitness, offspring[i]));
	}
	population.clear();
	offspring.clear();
	while(population.size() < N){
	   population.push_back(all.top().second);
	   all.pop();
	}
	while(!all.empty()){
		delete(all.top().second);
		all.pop();
	}
}
